...
const number_of_tags = 7;
const subSheetName = "Template"; // Replace this with your subsheet Name
...
function transposeRowsToColumns() {
  ...
  if (sheet) {
    i = 0;
    while (startRow + i * 12 <= numRows) {
      var sourceRange = sheet.getRange(startRow + i * 12, 3, 1, 4); // Adjust as needed
...
function placeTrueOption() {
  ...
  var numRows = sheet.getLastRow();
  var startRow = 2; // Starting row for your data
  var target_start_row = 10;

  if (sheet) {
    var i = 0;
    while (startRow + i * 12 <= numRows) {
      var sourceRange = sheet.getRange(startRow + i * 12, 8, 1, 1); // Adjust as needed
...
function addQuestionDefaultColumn() {
  ...
  sheet
    .getRange(2, 26, explanation_content_type.length, 1)
    .setValues(explanation_content_type);
}
...
function addTags() {
  ...
  if (sheet) {
    for (var i = 2; i <= numRows; i++) {
      ...
      var sub_topic = sheet.getRange("J" + i).getValue();
      var tag_name_4 = sheet.getRange("X" + (i + 4));

      var difficulty = sheet.getRange("K" + i).getValue();
      var tag_name_3 = sheet.getRange("X" + (i + 3));

      var tag_name_5 = sheet.getRange("X" + (i + 5));
      var tag_name_6 = sheet.getRange("X" + (i + 6));
      var tag_name_7 = sheet.getRange("X" + (i + 7));

      if (tag_name_count === 7) {
        tag_name_1.setValue("POOL_1");
        tag_name_5.setValue("IN_OFFLINE_EXAM");
        tag_name_6.setValue("COMPANY_");
        tag_name_7.setValue("SOURCE_");
      }
      if (topic) {
        tag_name_2.setValue("TOPIC_" + topic.toUpperCase() + "_MCQ");
      }
      if (difficulty) {
        tag_name_3.setValue("DIFFICULTY_" + difficulty.toUpperCase());
      }
      if (sub_topic) {
        tag_name_4.setValue("SUB_TOPIC_" + sub_topic.toUpperCase() + "_MCQ");
      }
    }
  } else {
    Logger.log("Sheet not found with the specified name.");
...
function deleteLastColumn() {
  ...
  if (lastColumn > 0) {
    ...
    // Log a message if there are no columns to delete
    Logger.log("No columns to delete.");
  }
}
...
function mcqFormatter() {
  ...
  var existingHeaders = targetSheet
  ...
  targetSheet
    .getRange(1, 1, 1, combinedHeaders.length)
    .setValues([combinedHeaders]);
  ...
  addDataToColumn();
  boldHeaderRow();
  deleteLastColumn(); //for deleting difficuilty column
}
